...
void CreateLines::setup()
{
    ...
    inputCount = 1;
    _keepScene = true;
    _myName = "Create/CreateLines";
    _myDesc = "Create lines from points";
}

void CreateLines::description(QString name, QString desc)
{
    setup();
}


QString CreateLines::description()
{
    setup();
    qDebug() << "Description return" << inputCount;
    return QString("Create/CreateLines;Create lines from points/1");
}

void CreateLines::knobs(Knob_Callback *f)
{
    ComboBox_knob(f, &_columnX, "X Coords: ", "col_x");
    ComboBox_knob(f, &_columnY, "Y Coords: ", "col_y");
    ComboBox_knob(f, &_columnID, "Line ID: ", "col_id");
    CheckBox_knob(f, &_useID, "Use ID values: ");
    CheckBox_knob(f, &_keepScene, "Keep prev. objects: ");
}


void CreateLines::engine()
{
    ...
    qDebug() << "CreateLines Engine finished!!!";
